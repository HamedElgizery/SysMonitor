  ...
  if [ "$selection" == "CPU Performance" ]; then
    ...
    while true; do
      if [ -f "get_gpu_stats/getgpu.sh" ]; then
        gpu_output=$($docker_call bash -c "get_gpu_stats/getgpu.sh" | tr -d '\r')
    ...
    while true; do
      if [ -f "get_disk_usage/getdisk.sh" ]; then
        disk_output=$($docker_call bash -c "get_disk_usage/getdisk.sh" | tr -d '\r')
    ...
    while true; do
      if [ -f "get_net_stats/getnet.sh" ]; then
        net_output=$($docker_call bash -c "get_net_stats/getnet.sh" | tr -d '\r')
    ...
    while true; do
      if [ -f "get_sys_load/getload.sh" ]; then
        sys_output=$($docker_call bash -c "get_sys_load/getload.sh" | tr -d '\r')
